...
from vail_scraper.errors import NoContentPageBug
...
class VailScraper:
    def __init__(self, database: aiosqlite.Connection, user_agent: str) -> None:
        ...
        self._rate_limiter: TimesPer = TimesPer(2, 3)
    ...
    async def _tick(self) -> None:
        ...
        while True:
            ...
            await self._database.executemany("insert or replace into users (id, name, last_scraped) values (?, ?, ?)", [(user.user_id, user.display_name, paged_scraped_at) for user in page])
            await self._database.executemany("insert or replace into stats (id, won, lost, draws, abandoned, kills, assists, deaths, points, game_hours) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [(user.user_id, user.stats.won, user.stats.lost, user.stats.draws, user.stats.abandoned, user.stats.kills, user.stats.assists, user.stats.deaths, user.stats.point, user.stats.game_hours) for user in page])
            await self._database.commit()

            page_id += 1

        self.last_scrape_at = time.time()
        result = await self._database.execute("select id, name from users where last_scraped < ?", [self.last_scrape_at])
    ...
    async def get_page(self, page_id: int, page_size: int = 100) -> list[ScoreLeaderboardPlayer]:
        error: Exception | None = None
        for retry_attempt in range(5):
            async with self._rate_limiter.acquire():
                _logger.debug("fetching page %s", page_id)
                try:
                    response = await self._session.get("/api/leaderboard", params={
