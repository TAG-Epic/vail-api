...
async def get_stats_for_user(request: web.Request) -> web.StreamResponse:
    ...
    user_stats = await vail_client.get_accelbyte_user_stats(user_id)
    if user_stats is None:
        return web.json_response({"detail": "user not found", "code": APIErrorCode.USER_NOT_FOUND})
        ...
        "general": {
            "stats": {
                "matches": {
                    ...
                    "abandoned": user_stats.get(AccelByteStatCode.GAMES_ABANDONED, 0)
                },
                "kills": user_stats.get(AccelByteStatCode.KILLS, 0),
                "deaths": user_stats.get(AccelByteStatCode.DEATHS, 0),
                "game_hours": float(user_stats.get(AccelByteStatCode.GAME_SECONDS, 0)) / 60 / 60
            },
            "meta": {
